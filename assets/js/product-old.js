...
function manage_price() {
        selected_verient = product_data.verients.find(verient =>
        verient.color === selected_color &&
        verient.storage === selected_storage &&
        verient.size === selected_size
    );

    selected_verient = filtered[0];

    $(".mrp").html(selected_verient.mrp);
    $(".price").html("&#8377;" + selected_verient.selling_price);

    var discount = Math.round(
        ((selected_verient.mrp - selected_verient.selling_price) / selected_verient.mrp) * 100
    );
    $(".discount").html(discount + "% off");

    $(".product-details").text(selected_verient.features);
}

function buyNow(brand,title,price,mrp,image) {
